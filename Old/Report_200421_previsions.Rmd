...
if(!is.na(firstcase)) {
    x<-1:length(firstcase:length(pays$dateRep))
    xmax[i]<-x[length(x)]
    y<-cumsum(pays$cases)[firstcase:length(pays$dateRep)]/pays$popData2018[1]*1e5
  lines(x,y)
  # text(x=length(x),y=y[length(y)],labels=as.character(size10$country[i]),cex=0.8,pos=4,col="red")
}
}
...
if(!is.na(firstcase)) {
    x<-1:length(firstcase:length(pays$dateRep))
    y<-log10(cumsum(pays$cases)[firstcase:length(pays$dateRep)]/pays$popData2018[1]*1e5+1)
  lines(x,y)
  # text(x=length(x),y=y[length(y)],labels=as.character(size10$country[i]),cex=0.8,pos=4,col="red")
}
}
